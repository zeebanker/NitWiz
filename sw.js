...
self.addEventListener('fetch', event => {
  // Only cache GET requests — skip POST (Firebase Auth/Firestore API calls)
  if (event.request.method !== 'GET') return;

  // Don't cache Firebase API calls
  const url = event.request.url;
  if (url.includes('googleapis.com') || url.includes('firebaseio.com') || url.includes('firestore.googleapis.com')) {
    return;
  }
